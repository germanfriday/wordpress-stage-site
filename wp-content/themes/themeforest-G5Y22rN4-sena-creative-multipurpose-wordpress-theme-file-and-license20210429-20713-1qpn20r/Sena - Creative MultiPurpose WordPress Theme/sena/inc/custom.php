<?php
// Breadcrumbs
if ( ! function_exists( 'sena_breadcrumbs' ) ) {
	function sena_breadcrumbs( ) {
		global $post, $sena_config;

		$text['home']     = ( ! empty( $sena_config['home-page-title'] ) ? $sena_config['home-page-title'] : esc_html__( 'Home', 'sena' ) );
		$text['category'] = '%s';
		$text['search']   = esc_html__( 'Search Results for Query &ndash; %s', 'sena' );
		$text['tag']      = esc_html__( 'Posts Tagged &ndash; %s', 'sena' );
		$text['author']   = esc_html__( 'Articles Posted by %s', 'sena' );
		$text['404']      = esc_html__( 'Error 404', 'sena' );
		$text['page']     = esc_html__( 'Page', 'sena' );
