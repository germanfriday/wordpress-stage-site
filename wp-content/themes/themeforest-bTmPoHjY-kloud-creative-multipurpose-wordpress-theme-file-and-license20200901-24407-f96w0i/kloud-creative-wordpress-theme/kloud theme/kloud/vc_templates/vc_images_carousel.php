			...
			<?php if ( 'yes' !== $hide_pagination_control ) :  ?>
				...
				<ol class="vc_carousel-indicators">
					<?php for ( $z = 0; $z < count( $images ); $z ++ ) :  ?>
						<li data-target="#<?php echowp_kses_post( $carousel_id) ?>" data-slide-to="<?php echo wp_kses_post($z) ?>"></li>
					<?php endfor; ?>
				</ol>
			<?php endif ?>
			<!-- Wrapper for slides -->
							...
							<div class="vc_item">
								<div class="vc_inner">
									...
									<?php elseif ( 'custom_link' === $onclick && isset( $custom_links[ $i ] ) && '' !== $custom_links[ $i ] ) :  ?>
										<a
											href="<?php echo wp_kses_post($custom_links[ $i ]) ?>"<?php echo( ! empty( $custom_links_target ) ? ' target="' . $custom_links_target . '"' : '' ) ?>>
											<?php echo wp_kses_post($thumbnail) ?>
										</a>
									<?php else : ?>
										<?php echo wp_kses_post($thumbnail) ?>
