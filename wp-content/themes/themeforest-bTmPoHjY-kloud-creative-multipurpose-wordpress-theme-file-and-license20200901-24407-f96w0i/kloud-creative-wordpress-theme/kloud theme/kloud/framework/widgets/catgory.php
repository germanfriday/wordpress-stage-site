...
class WP_Widget_Categories_Custom extends WP_Widget {
  ...
  function __construct() {
    $widget_ops = array( 'classname' => 'widget_categories widget_categories_custom', 'description' => __( "A list of categories, with slightly tweaked output.", 'kloud'  ) );
    parent::__construct( 'categories_custom', __( 'Categories Custom', 'kloud' ), $widget_ops );
  }

  function widget( $args, $instance ) {
    extract( $args );

    $title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Categories Custom', 'kloud'  ) : $instance['title'], $instance, $this->id_base);
  ...
  function form( $instance ) {
    ...
    $dropdown = false;
?>
    <p><label for="<?php echo wp_kses_post($this->get_field_id('title', 'kloud' )); ?>"><?php _e( 'Title:', 'kloud'  ); ?></label>
    ...
    <label for="<?php echo wp_kses_post($this->get_field_id('count')); ?>"><?php _e( 'Show post counts', 'kloud'  ); ?></label><br />
<?php
  }
