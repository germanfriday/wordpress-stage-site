...
function custom_nav_update($menu_id, $menu_item_db_id, $args ) {
    $fields = array('submenu_type','dropdown','widget_area', 'submenu_size' , 'column_width','group','hide_link','bg_image','bg_image_attachment','bg_image_size','bg_image_position','bg_image_repeat','bg_color','menu_icon');
...
function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = sprintf( __( '%s (Invalid)', 'kloud'), $item->title );
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = sprintf( __('%s (Pending)', 'kloud'), $item->title );
    }
    ...
    <li data-menuanchor="" id="menu-item-<?php echo esc_attr( $item_id ); ?>" class="<?php echo implode(' ', $classes ); ?>">
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','kloud'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','kloud'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item','kloud'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php _e( 'Edit Menu Item' ,'kloud'); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Navigation Label' ,'kloud'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Title Attribute','kloud' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php _e( 'Open link in a new window/tab' ,'kloud'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'CSS Classes (optional)' ,'kloud'); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Link Relationship (XFN)' ,'kloud'); ?><br />
            ...
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Add Shortcode for mega menu' ,'kloud'); ?><br />
                    ...
                    <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.','kloud'); ?></span>
                </label>
            </p>
            <?php }
            /*
			...
			<p class="description description-thin">
				...
				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
				     	<option value="standard" <?php echo wp_kses_post(( $value == 'standard' ) ? ' selected="selected" ' : ''); ?>><?php _e( 'Standard Dropdown', 'kloud' ); ?></option>
                        <option value="mega_menu" <?php echo wp_kses_post(( $value == 'mega_menu' ) ? ' selected="selected" ' : ''); ?>><?php _e( 'Mega Dropdown', 'kloud' ); ?></option>
					</select>
				</label>
			</p>
            <?php
		    ...
		    <p class="field-submenu_size description description-thin">
                <label for="edit-menu-item-submenu_size-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Add Width Mega Menu ' ,'kloud'); ?><br />
        ...
        <p class="description description-wide">
            <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                ...
                <input alt="#TB_inline?height=400&width=500&inlineId=<?php echo esc_attr( $key . '-' . $item_id . '-popup' ); ?>" title="<?php _e( 'Click to browse icon', 'kloud' ) ?>" class="thickbox button-secondary submit-add-to-menu" type="button" value="<?php _e( 'Browse Icon', 'kloud' ) ?>" />
            ...
            <p class="description description-wide">
                <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                    <?php echo esc_html( $title ); ?><br />
                    <input type="text" value="<?php echo esc_attr( $value ); ?>" id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>" />
                    <button id="browse-edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php _e( 'Browse Image', 'kloud' ); ?></button>
					<a class="button btn_clear button-primary" href="javascript: void(0);">Clear</a>
                </label>
            </p>
                ...
                <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                    <select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
                        ...
                        foreach ( $options as $option ) {
                            ?>
                            <option value="<?php echo esc_attr( $option ); ?>" <?php echo wp_kses_post(( $value == $option ) ? ' selected="selected" ' : ''); ?>><?php echo esc_html( $option ); ?></option>
                        <?php
                        }
                        ?>
                    </select>
                ...
                <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                    <select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
                        ...
                        foreach ( $options as $op_value => $op_text ) {
                            ?>
                            <option value="<?php echo esc_attr( $op_value ); ?>" <?php echo wp_kses_post(( $value == $op_value ) ? ' selected="selected" ' : ''); ?>><?php echo esc_html( $op_text ); ?></option>
                        <?php
                        }
                        ?>
                    </select>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( __('Original: %s', 'kloud'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php _e('Remove','kloud'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php _e('Cancel','kloud'); ?></a>
            </div>
...
class HeroMenuWalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( !empty( $label ) ) {
				...
				switch ( $label ) {
					case 'hot':
						$label_text = esc_html__('Hot', 'kloud');
					break;
					case 'sale':
						$label_text = esc_html__('Sale', 'kloud');
					break;
					case 'new':
						$label_text = esc_html__('New', 'kloud');
					break;
				}
