 ...
 * @version 1.0
 */
class kloud_WooCommerce {
    
	/**
	 * Construction function
	 *
	 * @since  1.0
	 * @return kloud_WooCommerce
	 */
	function __construct() {
        ...
        add_filter( 'kloud_after_single_product_image', array( $this, 'product_thumbnails' ) );
        // Add Shop Toolbar
	...
	function variation_attribute_options( $args ) {
		...
		if ( function_exists( 'wc_attribute_label' ) && $attribute ) {
			$args['show_option_none'] = esc_html__( 'Select', 'kloud' ) . ' ' . wc_attribute_label( $attribute );
		}
	...
	public function instance_search_result() {
		...
		$response    = array();
		$products    = array_merge(  $products_s );
		$product_ids = array();
		...
		if ( empty( $response ) ) {
			$response[] = sprintf( '<li>%s</li>', esc_html__( 'Nothing found', 'kloud' ) );
		}
	...
	function product_thumbnails() {
        ...
        if($thumb_position == 'left' || $thumb_position == 'right') {
           $data_slick = 'data-slick=\'{"slidesToShow": 5,"slidesToScroll": 1,"asNavFor": "#product-images","arrows": false, "vertical":true ,  "focusOnSelect": true,  "responsive":[{"breakpoint": 736,"settings":{"slidesToShow": 4, "vertical":false}}]}\''; 
            ...
            $data_slick = 'data-slick=\'{"slidesToShow": 5,"slidesToScroll": 1,"asNavFor": "#product-images","arrows": false, "focusOnSelect": true,  "responsive":[{"breakpoint": 736,"settings":{"slidesToShow": 5, "vertical":false}}]}\''; 
        }else {
            $data_slick = ''; 
        }
	...
	function shop_toolbar() {
		...
		$output = array();
		$filters = '';
		$filters = sprintf( '<a href="#" class="jws-filter filters"><span>+</span> %s</a>', esc_html__( 'Filters', 'kloud' ) );
		$found = '';
		$shop_view = '';
        ...
        $found = '';
			...
			if ( $wp_query && isset( $wp_query->found_posts ) ) {
				$found = ' '.esc_html__( 'Showing', 'kloud' ).' <span>' . $wp_query->found_posts . ' </span>'.esc_html__( 'results', 'kloud' ).'';
			}
			if ( $found ) {
		...
		<div id="jws-shop-topbar" class="widgets-area shop-topbar <?php echo esc_attr( $columns_class ); ?>">
			<div class="shop-topbar-content">
				...
				<div class="shop-filter-actived">
					...
					global $wp_query;
					if ( $wp_query && isset( $wp_query->found_posts ) ) {
						echo ''. esc_html__( 'Showing ', 'kloud' ) . $wp_query->found_posts . esc_html__( ' results', 'kloud' );
					}
