...
function jwsthemes_get_rating_html( $rating_html, $rating ) {
	...
	$rating_html = '';
			...
			$rating_html  = '<div class="star-rating" title="' . sprintf( __( 'Rated %s out of 5', 'kloud' ), $rating ) . '">';

			$rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . __( 'out of 5', 'kloud' ) . '</span>';
...
if ( ! function_exists( 'tb_woocommerce_page_title' ) ) {
	...
	function jwsthemes_woocommerce_page_title() {
		...
		if ( is_search() ) {
			$page_title = sprintf( __( 'Search Results: &ldquo;%s&rdquo;', 'kloud' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( __( '&nbsp;&ndash; Page %s', 'kloud' ), get_query_var( 'paged' ) );
			...
			$page_title = __( 'Archives Products', 'kloud' );

		} elseif ( is_single() ) {

			$page_title = __( 'Single Product', 'kloud' );
...
function wc_cart_totals_coupon_html_custom( $coupon ) {
	...
	if ( $coupon->enable_free_shipping() ) {
		$value[] = __( 'Free shipping coupon', 'kloud' );
    }
...
?>
		...
		if ( has_post_thumbnail() ) {
			...
			$html .= sprintf( '<img src="%s" alt="%s" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'kloud' ) );
			$html .= '</div>';
		}
 ...
 *
 */
function jws_kloud_wc_change_product_per_page() {
	$number = cs_get_option( 'wc-number-per-page' );
	return $number;
}
add_filter( 'loop_shop_per_page', 'jws_kloud_wc_change_product_per_page' , 20 );
/** 
...
function isa_wc_additional_info_tab_link_text( $text, $tab_key ) {
 ...
 function jws_kloud_wc_currency() {
	if ( ! class_exists( 'kloud_Addons_Currency' ) ) return;
	$currencies = kloud_Addons_Currency::getCurrencies();
					
	if ( count( $currencies > 0 ) ) :
		$woocurrency = kloud_Addons_Currency::woo_currency();
		$woocode = $woocurrency['currency'];
		if ( ! isset( $currencies[$woocode] ) ) {
			$currencies[$woocode] = $woocurrency;
		}
		$default = kloud_Addons_Currency::woo_currency();
	...
	return apply_filters( 'jws_kloud_wc_currency', $output );
}
/**
 ...
 * @since 1.0.0
 */
if ( ! function_exists( ' jws_kloud_shopping_cart' ) ) {
	function  jws_kloud_shopping_cart() {
		global $woocommerce;
		...
		$output .= '<div  class="jws-icon-cart">';
			$output .= '<a class="cart-contents" href="#" title="' . esc_html( 'View your shopping cart', 'kloud' ) . '">';
				$output .= '<span class="pe-7s-cart"></span>';
				$output .= nm_get_cart_contents_count();
			$output .= '</a>';
		$output .= '</div>';
		return apply_filters( ' jws_kloud_shopping_cart', $output );
	}
}
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'jws_kloud_wc_my_account' ) ) {
	function jws_kloud_wc_my_account() {
		$output = '';
		...
		$output .= '<div class=" '.$loged.' jws-my-account  ">';
			$output .= '<a class="account" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '"><span class="hidden-xs hidden-sm '.$hidden_md.'">'.esc_html('account' , 'kloud').'</span><i class="clo25 ml fz24 midde pe-7s-user"></i><span class="acount-tt">'.$name.'</span></a>';
			$output .= '<ul class="account-content">';
				if ( is_user_logged_in() ) {
					$output .= '<li class="button button--white"><a class="item-ac" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_html__( 'Dashboard', 'kloud' ) . '</a></li>';
					$output .= '<li class="button button--white"><a class="item-ac" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_html__( 'Order Tracking', 'kloud' ) . '</a></li>';
					$output .= '<li class="button button--white"><a class="item-ac" href="' . esc_url( wc_logout_url(  ) ) . '">' . esc_html__( 'Logout', 'kloud' ) . '</a></li>';
				} else {
					$output .= '<li class="button button--white"><a class="item-ac " href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_html__( 'Login', 'kloud' ) . '</a></li>';
                    $output .= '<li class=" button button--white-reversed"><a class="item-ac " href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_html__( 'Sign up', 'kloud' ) . '</a></li>';
				}
			$output .= '</ul>';
		$output .= '</div>';

		return apply_filters( 'ws_kloud_wc_my_account', $output );
	}
}
...
if( ! function_exists( 'kloud_wishlist_btn' ) ) {
	function kloud_wishlist_btn() {
...
if( ! function_exists( 'kloud_configure_compare' ) ) {
	add_action( 'init', 'kloud_configure_compare' );
	function kloud_configure_compare() {
        ...
        if ( get_option('yith_woocompare_compare_button_in_product_page') == 'yes' ) {
        	add_action( 'woocommerce_single_product_summary', 'kloud_before_compare_button', 33 );
        	add_action( 'woocommerce_single_product_summary', 'kloud_after_compare_button', 37 );
        }

	}
}

if( ! function_exists( 'kloud_before_compare_button' ) ) {
	function kloud_before_compare_button() {
		echo '<div class="compare-btn-wrapper">';
	}
}

if( ! function_exists( 'kloud_after_compare_button' ) ) {
	function kloud_after_compare_button() {
		echo '</div>';
	}
}

if( ! function_exists( 'kloud_compare_btn' ) ) {
	function kloud_compare_btn() {
		...
		echo '<div class="product-compare-button">';
            ...
            if ( ! isset( $button_text ) || $button_text == 'default' ) {
                $button_text = get_option( 'yith_woocompare_button_text', __( 'Compare', 'kloud' ) );
            ...
            printf( '<a href="%s" class="%s" data-product_id="%d" rel="nofollow">%s</a>', kloud_compare_add_product_url( $product_id ), 'compare' . ( $is_button == 'button' ? ' button' : '' ), $product_id, $button_text );
...
if( ! function_exists( 'kloud_compare_add_product_url' ) ) {
    function kloud_compare_add_product_url( $product_id ) {
...
if( ! function_exists( 'kloud_compare_styles' ) ) {
	add_action( 'wp_print_styles', 'kloud_compare_styles', 200 );
	function kloud_compare_styles() {
		if( ! class_exists( 'YITH_Woocompare' ) ) return;
...
if( ! function_exists( 'kloud_add_image_size' ) ) {
	add_action( 'after_setup_theme', 'kloud_add_image_size' );

	function kloud_add_image_size() {
...
if ( ! function_exists( 'kloud_template_loop_product_thumbnail' ) ) {
	function kloud_template_loop_product_thumbnail() {
		echo kloud_get_product_thumbnail();
	}
}

if ( ! function_exists( 'kloud_get_product_thumbnail' ) ) {
	function kloud_get_product_thumbnail( $size = 'shop_catalog_2', $attach_id = false ) {
		global $post, $kloud_loop;

		if( ! empty( $kloud_loop['double_size'] ) && $kloud_loop['double_size'] ) {
		...
		if ( has_post_thumbnail() ) {
			...
			if( function_exists( 'wpb_getImageBySize' ) ) {
				...
				if( ! empty( $kloud_loop['img_size'] ) ) {
					$size = $kloud_loop['img_size'];
				}  
...
if( ! function_exists( 'kloud_woocommerce_installed' ) ) {
	function kloud_woocommerce_installed() {
...
if( ! function_exists( 'kloud_category_thumb_double_size' ) ) {
	function kloud_category_thumb_double_size( $category ) {
		global $kloud_loop;
...
add_action( 'woocommerce_before_subcategory_title', 'kloud_category_thumb_double_size', 10 );
/**
 * ------------------------------------------------------------------------------------------------
...
if( ! function_exists( 'kloud_product_categories_nav' ) ) {
	function kloud_product_categories_nav() {
		global $wp_query, $post;

		//$show_subcategories = kloud_get_opt( 'shop_categories_ancestors' );
		...
		echo '<a href="#" class="kloud-show-categories">' . __('Categories', 'kloud') . '</a>';

		echo '<ul class="kloud-product-categories">';
		
		echo '<li class="cat-link shop-all-link"><a href="' . esc_url( $shop_link ) . '">' . __('All', 'kloud') . '</a></li>';

		if( $show_subcategories ) {
		//	kloud_show_category_ancestors();
		} else {
...
/**
	...
	function get_categories_filter() {
		...
		$filters = '';
		$output  = array();
		$number  = apply_filters( 'kloud_product_cats_filter_number', 4 );
    ...
    function get_colunm_shop() { 
        ...
        ?>
            ...
            <span><?php esc_html_e('See' , 'kloud') ?></span>
 ...
 * @since 1.0.0
 */
function jws_kloud_wc_add_extra_link_after_cart() {
	// Get page options
    ...
    if($size_guide) {
		...
		if ( ! empty( $size_guide ) ) {
			echo '<a class="jws-magnific-image" href="' . esc_url( $size_guide ) . '">' . esc_html__( 'What my size?', 'kloud' ) . '</a>';
		}
	echo '</div>';
    ?>
...
function jws_kloud_wc_change_image_thumbnail_size( $size ) {
...
add_filter( 'single_product_archive_thumbnail_size', 'jws_kloud_wc_change_image_thumbnail_size' );
/**
 * ------------------------------------------------------------------------------------------------
 * My account sidebar
 * ------------------------------------------------------------------------------------------------
...
if( ! function_exists( 'kloud_before_my_account_navigation' ) ) {
	function kloud_before_my_account_navigation() {
		echo '<div class="jws-my-account-sidebar">';
		the_title( '<h3 class="woocommerce-MyAccount-title entry-title">', '</h3>' );
	}

	add_action( 'woocommerce_account_navigation', 'kloud_before_my_account_navigation', 1 );
}

if( ! function_exists( 'kloud_after_my_account_navigation' ) ) {
	function kloud_after_my_account_navigation() {
		...
		<?php endif;
		echo '</div><!-- .kloud-my-account-sidebar -->';
	}

	add_action( 'woocommerce_account_navigation', 'kloud_after_my_account_navigation', 30 );
}
/**
        ...
        if ( is_array( $cart_totals ) ) {
            foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
                ...
                if ( $_product->is_sold_individually() && $quantity > 1 ) {
                    //wc_add_notice( sprintf( __( 'You can only have 1 %s in your cart.', 'kloud' ), $_product->get_title() ), 'error' );
                ...
                if ( $passed_validation ) {
                    ...
                    // Code from "../kloud/woocommerce/cart/cart.php" (variable names changed)
