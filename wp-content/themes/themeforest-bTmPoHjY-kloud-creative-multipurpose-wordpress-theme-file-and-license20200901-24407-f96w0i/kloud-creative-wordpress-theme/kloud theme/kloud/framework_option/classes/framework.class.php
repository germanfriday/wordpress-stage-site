...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'kloud' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'kloud' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'kloud' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo '<h1>'.'kloud 1.0.6'.'</h1>';
      echo '<fieldset>';

      echo wp_kses_post(( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">'. __( 'Settings saved.', 'kloud' ) .'</span>' : '');

      submit_button( __( 'Save', 'kloud' ), 'primary cs-save', 'save', false, array( 'data-save' => __( 'Saving...', 'kloud' ) ) );
      submit_button( __( 'Restore', 'kloud' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( __( 'Reset All Options', 'kloud' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. __( 'show all options', 'kloud' ) .'</a>' : '';
      ...
      echo '<div class="cs-block-left">Developed by the JWSThemes team.</div>';
