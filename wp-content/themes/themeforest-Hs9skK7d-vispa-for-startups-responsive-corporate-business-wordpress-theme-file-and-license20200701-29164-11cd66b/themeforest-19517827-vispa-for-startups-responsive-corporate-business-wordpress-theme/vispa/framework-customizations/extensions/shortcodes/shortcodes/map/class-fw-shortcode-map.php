...
class FW_Shortcode_Map extends FW_Shortcode {
		...
		if ( empty( $this->data ) ) {
			$this->data = apply_filters( 'fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'    => __( 'Custom', 'vispa' ),
					'options'  => array(
						'locations' => array(
							'label'         => __( 'Locations', 'vispa' ),
							'popup-title'   => __( 'Add/Edit Location', 'vispa' ),
							'type'          => 'addable-popup',
							'desc'          => false,
							'template'      => '{{  if (location.location !== "") {  print(location.location)} else { print("' . __( 'Note: Please set location', 'vispa' ) . '")} }}',
							'popup-options' => array(
								'location'    => array(
									'type'  => 'map',
									'label' => __( 'Location', 'vispa' ),
								),
								'title'       => array(
									'type'  => 'text',
									'label' => __( 'Location Title', 'vispa' ),
									'desc'  => __( 'Set location title', 'vispa' ),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => __( 'Location Description', 'vispa' ),
									'desc'  => __( 'Set location description', 'vispa' )
								),
								'url'         => array(
									'type'  => 'text',
									'label' => __( 'Location Url', 'vispa' ),
									'desc'  => __( 'Set page url (Ex: http://example.com)', 'vispa' ),
								),
								'thumb'       => array(
									'label' => __( 'Location Image', 'vispa' ),
									'desc'  => __( 'Add location image', 'vispa' ),
									'type'  => 'upload',
								)
							)
						)
	...
	protected function _render( $atts, $content = null, $tag = '' ) {
		if ( ! isset( $atts['data_provider']['population_method'] ) ) {
			trigger_error(
				__( 'No location provider specified for map shortcode', 'vispa' )
			);
		...
		if ( ! isset( $this->data[ $provider ] ) ) {
			trigger_error(
				sprintf( __( 'Unknown location provider "%s" specified for map shortcode', 'vispa' ), $provider )
			);
