<?php 
$audio_type       = airtheme_get_post_meta(get_the_ID(), 'theme_meta_audio_type');
$audio_soundcloud = airtheme_get_post_meta(get_the_ID(), 'theme_meta_audio_soundcloud');
...
<?php }else{
	$audio_artist = airtheme_get_post_meta(get_the_ID(), 'theme_meta_audio_artist');
	$audio_mp3    = airtheme_get_post_meta(get_the_ID(), 'theme_meta_audio_mp3');

    if ($audio_mp3) { ?>

    <div class="content-audio">
        <?php if($audio_artist){ ?><cite class="content-audio-artist"><?php esc_html_e('Artist:','air-theme'); ?> <?php echo esc_html($audio_artist); ?></cite><?php } ?>
        <ul class="audio_player_list ">
            <?php foreach($audio_mp3['name'] as $i => $name){
                $url = esc_url($audio_mp3['url'][$i]); ?>
